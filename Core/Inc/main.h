...
/* PS2/2.4G 手柄接收器 ATT/CS 片选脚，低电平开始一次手柄读取。
 * MC02 板子图上 SPI3 旁边有 GPIO PE14，这里默认用 PE14 做 ATT。
 * CLK/CMD/DAT 使用板子 SPI3 这一排物理引脚，但在手柄驱动里会重配成 GPIO
 * 软件模拟 PS2 时序，避免硬件 SPI 边沿不兼容。
 */
#define PS2_ATT_Pin GPIO_PIN_14
#define PS2_ATT_GPIO_Port GPIOE
